...
export class RegistroHorariosComponent implements OnInit {
    ...
    for (let i = 0; i < semana.length; i++) { // 12|05|2025 - 18|05|2025
      ...
      for (let j = 0; j < diaSemana.length; j++) { // LUNES
        ...
        for (let k = 0; k < this.horariosList.length; k++) { // 8:00am - 9:25am
          ...
          // }
          const esSemanaActual = semana[i].id === semana_actual;

          if (esSemanaActual) {
            const valorMatriz = matrizAccesos[this.listService.diasIndice(diaActualNombre)][this.listService.diasIndice(diaSemana[j].id)];
          ...
          if (buscarSemana.length > 0 && Object.keys(buscarSemana[0].horarios).length > 0) {
            ...
            // console.log('searchHorario', searchHorario);
  ...
  async guardarHorarios() {
    ...
    if (confirmed) {
      ...
      const dataSemanaAgregados: any = {};
      const dataSemanaEliminados: any = {};

      for (let semana = 0; semana < this.semanasList.length; semana++) {

        const dataHorarioAgregados: any[] = [];
        const dataHorarioEliminados: any[] = [];

        for (let dia = 0; dia < this.diasList.length; dia++) {
          for (let horario = 0; horario < this.horariosList.length; horario++) {

            const valueHorarioInicial = this.dataBase[this.semanasList[semana].id][this.diasList[dia].id][this.horariosList[horario]];
            const valueHorarioFinal = this.data[this.semanasList[semana].id][this.diasList[dia].id][this.horariosList[horario]];

            if (valueHorarioInicial.checked && !valueHorarioFinal.checked) {
              dataHorarioEliminados.push(`${this.diasList[dia].id}|${this.horariosList[horario]}|${valueHorarioInicial.type}`);
            } else if (!valueHorarioInicial.checked && valueHorarioFinal.checked) {
              dataHorarioAgregados.push(`${this.diasList[dia].id}|${this.horariosList[horario]}|${valueHorarioFinal.type}`);
            }

          }
        }
        dataSemanaAgregados[this.semanasList[semana].id] = dataHorarioAgregados;
        dataSemanaEliminados[this.semanasList[semana].id] = dataHorarioEliminados;
      }

      console.log('dataSemanaAgregados', dataSemanaAgregados);
      console.log('dataSemanaEliminados', dataSemanaEliminados);

      const body = {
        tipo: 'guardarHorarios',
        profesor: localStorage.getItem('correo'),
        agregados: dataSemanaAgregados,
        eliminados: dataSemanaEliminados
      };
